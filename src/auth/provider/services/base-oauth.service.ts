...
export class BaseOAuthService {
  ...
  protected async extractUserInfo(data: any) {
    return {
      ...data,
  ...
  public async findUserByCode(code: string): Promise<TypeUserInfo> {
    ...
    if (!tokensRequest.ok) {
      throw new BadRequestException(`Could not find user with ${this.options.profile_url}. Check if token is valid.`);
    }
    const tokens = await tokensRequest.json();
    if (!tokens.access_token) {
      throw new BadRequestException(`No tokens with ${this.options.access_url}. Check if authorization code is valid.`);
    }
    ...
    if (!userRequest.ok) {
      throw new UnauthorizedException(`Could not get user with ${this.options.profile_url}. Check if token is correct.`);
    }
